...
interface PageProps { params: { boardId: string }; }
...
export default function BoardPage({ params }: PageProps) {
  ...
  if (error) return (
    <div className="flex items-center justify-center h-full">
      <div className="text-center">
        <p className="text-red-400 font-medium">Failed to load board</p>
        <p className="text-slate-500 text-sm mt-1">{error.message}</p>
        <button onClick={() => refetch()} className="mt-4 text-blue-400 hover:text-blue-300 text-sm">Try again</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: board.background || "#0f172a" }}>
